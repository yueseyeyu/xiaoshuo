...
(function() {
  ...
  if (riskEl) {
    ...
    chart1.setOption({
      ...
      grid: { left: '12%', right: '8%', top: '15%', bottom: '8%' },
      xAxis: {
        type: 'category',
        data: [
          'R1 GLM\n循环论证', 'R2 末日\n乐园S级', 'R3 长夜\n余火S级', 'R4 T1\n采样率',
  ...
  if (sampleEl) {
    ...
    var books = [
      '第九特区', '末日乐园', '末日蟑螂', '限制级末日症候', '末世魔神游戏',
      '废土崛起', '末世大回炉', '黑暗血时代', '黑暗文明', '全球变异',
      '第一序列', '全球进化', '末世超级商人', '神秘尽头', '蹉跎'
    ];
    var rates = [30.6, 27.0, 26.8, 25.3, 22.0, 21.5, 21.7, 10.0, 10.0, 10.0, 10.0, 10.0, 0.9, 3.3, 4.1];

    chart2.setOption({
      tooltip: { trigger: 'axis', appendToBody: true },
      grid: { left: '8%', right: '5%', top: '8%', bottom: '8%' },
      xAxis: {
        type: 'category',
        data: books,
      ...
      series: [
        {
          ...
          data: rates.map(function(r) {
            return {
              value: r,
              itemStyle: { color: r > 15 ? accent2 : r < 5 ? accent2 : accent }
            };
          }),
          markLine: {
