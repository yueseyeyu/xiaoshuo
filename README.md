...
核心原则：**AI辅助构思+质量门禁管控**。AI可生成正文参考/草稿/对比版，但须通过S3质量门禁(PASS/WARNING/BLOCK)，未过门禁内容不直接采用（可用于灵感/推演/对比），作者有最终采用权。**所有建议必须有精品书量化数据支撑**。详见 [AI_PROTOCOL.md](AI_PROTOCOL.md) v1.2。
...
> v8.2 新增：API 模块 (`src/xiaoshuo/api/`) + canon 设定管线 + RP 推演引擎 + ngram-mod 自推测解码 + prototype 前端原型。v7.7 拆书指标 40+（18 类爽点 + 即时爽/延迟爽标签）。
