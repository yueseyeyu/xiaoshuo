...
for book in sorted(ALL_BOOKS.keys()):
    ...
    if score_files:
        for sf in score_files:
            sf.unlink()
        print(f"  {book}: 清理 {len(score_files)} 个评分文件")
        total_cleaned += len(score_files)
print(f"总计清理: {total_cleaned} 个文件")
...
lines.append("")
lines.append("| # | 书名 | 批次目录 | 总批次 | 待处理 |")
lines.append("|---|------|----------|--------|--------|")
for i, b in enumerate(book_list, 1):
    lines.append(f"| {i} | {b['name']} | {b['batch_dir']} | {b['total']} | {b['pending']} |")
lines.append("")
lines.append("## 评分维度")
...
print(f"\n已生成: {out_path}")
print(f"总批次: {total_batches}, 待处理: {total_pending}")
print(f"\n书籍清单 (按待处理量排序):")
for b in book_list:
    print(f"  {b['name']:<28} {b['pending']:>4}批待处理 (总{b['total']}批)")
