...
输出紧凑JSON(单字母键名,无换行,只输出JSON):
{{"b":"{book_name}","c":"{start}-{end}","wc":"字数","h":[{{"ch":章号,"t":"悬念/冲突/情感/信息","d":"描述","r":false}}],"cc":[{{"n":"角色名","ch":章号,"e":"事件","a":"上升/下降/转折/出场"}}],"ke":["事件1","事件2"],"ec":[{{"ch":章号,"e":"爽/抑/平/转/悲/燃","i":1-10}}],"cf":[{{"ch":章号,"t":"人物/势力/内部/环境","d":"描述","i":1-10}}],"fw":[{{"id":"ID","ch":章号,"d":"伏笔描述","s":"埋设/推进/回收"}}],"pn":"节奏特点","cs":[{{"ch":章号,"s":"1-2句摘要"}}]}}"""


# Compact key → full key mapping for L1 output normalization
_L1_KEY_MAP = {
    "b": "book", "c": "chapters", "wc": "word_count",
    "h": "hooks", "cc": "character_changes", "ke": "key_events",
    "ec": "emotion_curve", "cf": "conflicts", "fw": "foreshadowing",
    "pn": "pacing_notes", "cs": "chapter_summaries",
}
_L1_NESTED_MAP = {
    "h": {"t": "type", "d": "desc", "r": "resolved"},
    "cc": {"n": "name", "e": "event", "a": "arc"},
    "ec": {"e": "emotion", "i": "intensity"},
    "cf": {"t": "type", "d": "desc", "i": "intensity"},
    "fw": {"s": "status"},
    "cs": {"s": "summary"},
}


def _expand_compact_keys(parsed, key_map, nested_map=None):
    """Expand single-letter keys back to full names for downstream compatibility."""
    if not isinstance(parsed, dict):
        return parsed
    result = {}
    for k, v in parsed.items():
        full_key = key_map.get(k, k)
        if isinstance(v, list) and nested_map and k in nested_map:
            result[full_key] = [
                _expand_nested(item, nested_map[k]) if isinstance(item, dict) else item
                for item in v
            ]
        else:
            result[full_key] = v
...
def _expand_nested(item, nested_key_map):
    """Expand nested dict keys."""
    result = {}
    for k, v in item.items():
        result[nested_key_map.get(k, k)] = v
    return result
...
class RecursiveSummarizer:
    ...
    def _progress_bar(self, done, total, chunk_times, t0, completed_books=None, book_index=1, total_books=1):
        """Print single-line progress with ETA and update frontend state."""
        ...
        # P3: update frontend pipeline state
        try:
            write_stage(
                stage="recursive_summarize",
                stage_num=6,
                total=8,
                percent=int(((book_index - 1) / total_books * 100) + (pct / total_books)),
                eta_seconds=int(eta_s) if chunk_times else None,
                current_book=self.book_name,
                current_task=f"L1 章节组摘要 {done}/{total} ({pct:.0f}%) | {eta_str}",
    ...
    def _process_l1_chunk(self, chunk, ci):
        ...
        raw = _llm_call(prompt, max_tokens=1000)
        parsed = _parse_json(raw)
        if parsed:
            # Expand compact single-letter keys to full names for downstream
            return _expand_compact_keys(parsed, _L1_KEY_MAP, _L1_NESTED_MAP)
        return None  # callback handles fallback

    def run(self, save=True, completed_books=None, book_index=1, total_books=1):
        """Run full 3-level recursive summarization with checkpoint resume."""
        if not self.chapters:
            print(f"[WARN] No chapters loaded for {self.book_name}")
            return None

        completed_books = completed_books or []
        ...
        l1_summaries = self._run_l1_if_needed(completed_books=completed_books, book_index=book_index, total_books=total_books)
        if not l1_summaries:
            return None
        ...
        if len(l1_summaries) >= 3:
            ...
            l2_summaries = self._level2(l1_summaries, completed_books=completed_books, book_index=book_index, total_books=total_books)
        ...
        if not self._checkpoint.get("l3_done"):
            ...
            try:
                write_stage(
                    ...
                    stage_num=6,
                    total=8,
                    percent=int(book_index / total_books * 100),
                    current_book=self.book_name,
                    current_task="L3 全书分析",
                    completed_books=completed_books,
                )
            except Exception:
                pass
    ...
    def _level2(self, l1_summaries, completed_books=None, book_index=1, total_books=1):
        ...
        for gi, group in enumerate(groups):
            ...
            try:
                write_stage(
                    ...
                    percent=int(((book_index - 1) / total_books * 100) + (pct / total_books)),
                    current_book=self.book_name,
                    current_task=f"L2 卷级合成 {gi+1}/{n_total} ({pct:.0f}%)",
                    completed_books=completed_books,
                )
            except Exception:
                pass
...
def main():
    ...
    if not _check_server():
        print("[FAIL] LLM server unreachable, start it first: scripts\\start_model.bat", file=sys.stderr)
        sys.exit(1)
    ...
    elif book_filter and book_filter != "all":
    ...
    # P3: announce recursive stage to frontend
    write_stage(
        stage="recursive_summarize",
        stage_num=6,
        total=8,
        percent=0,
        current_task=f"准备处理 {n_total} 本书",
        completed_books=completed_books,
    )
    ...
    for bi, txt_path in enumerate(txt_files, 1):
        ...
        # P3: update stage with current book
        write_stage(
            stage="recursive_summarize",
            stage_num=6,
            total=8,
            percent=int((bi - 1) / n_total * 100),
            current_book=book_name,
            current_task=f"开始处理 {book_name}",
            completed_books=completed_books,
        )
        ...
        try:
            ...
            summarizer.load_chapters(chapters)
            result = summarizer.run(completed_books=completed_books, book_index=bi, total_books=n_total)
        except Exception as e:
            msg = f"{book_name} 递归摘要异常: {e}"
            print(f"  [FAIL] {msg}")
            mark_error("recursive_summarize", msg, stage_num=6, total=8)
            failed += 1
            continue
        ...
        if result is None:
            ...
            if summarizer.was_skipped:
                skipped += 1

        # P3: refresh progress after book
        ...
        write_stage(
            ...
            stage_num=6,
            total=8,
            percent=int(done / n_total * 100),
            eta_seconds=int(eta_remaining),
            current_task=f"已完成 {done}/{n_total} 本 | 预计剩余 {int(eta_remaining/60)} 分钟",
            completed_books=completed_books,
        )
...
if __name__ == "__main__":
    try:
        ...
        mark_error("recursive_summarize", str(e), stage_num=6, total=8)
        raise
