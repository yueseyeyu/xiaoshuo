...
_logger = get_logger("model_orchestrator")
...
def _load_config() -> dict:
    """加载 config.yaml 中的 model_orchestration 部分（v7.5: 使用 config_manager）。"""
    return get_config().get("model_orchestration", {})


# ============================================================
# 工具函数：清理 Qwen3.5 的 empty  thinking 标签
# ============================================================
# v7.5: 长期方案是用 llama-server 的 --chat-template 参数控制输出格式，
# 此处作为兼容性 fallback 保留。可通过 config.yaml 的 model_orchestration
# 中 models.{name}.strip_thinking 控制是否启用（默认 true）。
def _strip_thinking_tags(text: str) -> str:
    """移除 Qwen3.5-9B 推理时产生的空  thinking... response 标签。

    Qwen3.5-9B-Instruct 在 chatml 下仍可能输出 thinking 空标签。
    此函数清理这些空标签，不影响正常内容。

    调用时机: 每个 chat() 返回前自动调用。
    """
    if not text:
        return text
...
class ModelServer:
    ...
    def __init__(self, model_key: str, model_config: dict):
        ...
        self.api_key = model_config.get("api_key", "")  # v7.5: 可选 API 认证
...
class ModelOrchestrator:
    ...
    def chat_with_trace(
        ...
        """v7.5: 带调用链追踪的推理。自动记录 caller → task_type → 耗时。

        Args:
            task_type: 任务类型
            messages: 聊天消息
            caller: 调用者标识（如 "world_builder.stage3"）
            max_tokens, temperature, timeout: 推理参数

        Returns:
            同 chat() 返回值，额外附加 _trace 字段:
            {"content": ..., "_trace": {"caller": ..., "task_type": ..., "elapsed_ms": ...}}
        """
        _logger.debug("[trace] %s → %s (tokens=%d, temp=%.2f)",
                       caller or "unknown", task_type, max_tokens, temperature)
        t0 = time.perf_counter()
        result = self.chat(task_type, messages, max_tokens, temperature, timeout)
        ...
        if "error" not in result:
            ...
            _logger.debug("[trace] %s ← %s (%d ms, %d tokens)",
                          caller or "unknown", task_type, elapsed_ms,
                          result.get("usage", {}).get("total_tokens", 0))
        else:
            _logger.warning("[trace] %s ← %s FAILED: %s",
                            caller or "unknown", task_type, result.get("error", ""))
        return result
