...
| 量化策略 | 按任务动态切换 IQ4_XS/Q4_K_M | 🔄 **单 GPU 顺序切换 (swap_to)** | v7.5 废弃双模型共存（8GB VRAM 不足），改为顺序切换 |
| S1 创意模型 | Qwen3.5-9B Q4_K_M | 🔄 Qwen3.5-9B 单模型（WebNovel专家模型 P0评估未采用） | v7.5 确认单模型方案 |
| DeepSeek-R1-Distill | P0 创意生成候选 | 🔄 **升级为 DeepSeek-R1-0528-Qwen3-8B**，交叉审查模型 | v7.5 角色重定位 + 模型升级 |
...
2. **合规安全**：S3 质量门禁 + 七层 AI 检测 + PAN 2026 外部基准
