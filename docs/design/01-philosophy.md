...
本项目是一个面向番茄小说网文创作的 **AI 辅助技术中台**。系统通过本地大语言模型（LLM）提供剧情引导、设定校验、风格检测、正文参考生成等能力，最终实现"AI 辅助构思+质量门禁管控+作者最终采用"的协作范式。

**关键上下文**：截至 2026 年 5 月，番茄平台已整治 15 万本 AI 生成作品，账号 855 个。AI 生成正文将导致降权、下架乃至封号。因此，AI 生成内容须通过 S3 质量门禁（PASS/WARNING/BLOCK），未过门禁内容不直接采用（可用于灵感/推演/对比），作者有最终采用权。详见 [AI_PROTOCOL.md](../../AI_PROTOCOL.md) v1.2。
...
| **合规保障** | AI 生成内容须过 S3 质量门禁，未过门禁不直接采用 | S4+++ 全部 SAFE + PAN 2026 基准验证 + S3 门禁 verdict |
...
| 合规 | AI 生成内容须过 S3 质量门禁，作者有最终采用权 | ★★★ |
| 技术栈 | Python 脚本 + 状态机 | ★★☆ |
| 前端 | Vue 3 + Vite + TS + Pinia，后端 FastAPI 8089 端口 | ★★☆ |
| 时间 | 每日约 2 小时，单周 ≤14h | ★☆☆ |
| MCP/API | 可选增强，非核心依赖 | ★☆☆ |
